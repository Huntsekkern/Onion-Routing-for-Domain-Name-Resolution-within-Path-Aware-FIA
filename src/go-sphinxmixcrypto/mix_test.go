...
func TestSphinxNodeReplay(t *testing.T) {
	...
	header, sharedSecrets, err := headerFactory.BuildHeader(route, route[len(route)-1][:], zeroDest)
	if err != nil {
		t.Fatalf("Unable to create forwarding header: %#v", err)
	}
	packetFactory := NewSphinxPacketFactory(&params, pki, randReader)
	message := []byte("the quick brown fox")
	fwdMsg, err := packetFactory.BuildForwardSphinxPacket(route, route[len(route)-1], message, header, sharedSecrets)
	if err != nil {
		t.Fatalf("Unable to create forwarding message: %#v", err)
	}
	// Allow the node to process the initial packet, this should proceed
	// without any failures.
	if _, _, err := SphinxPacketUnwrap(&params, replayCache, &keyState, fwdMsg); err != nil {
	...
	if _, _, err := SphinxPacketUnwrap(&params, replayCache, &keyState, fwdMsg); err != ErrReplayedPacket {
...
func MixStateMachine(firstHop [16]byte, replayCacheMap map[[16]byte]*SimpleReplayCache, keyStateMap map[[16]byte]*SimpleKeyState, sphinxPacket *SphinxPacket, expected ExpectedState) (*UnwrappedMessage, error) {
	...
	for {
		...
		unwrappedMessage, _, err = SphinxPacketUnwrap(&params, replayCache, keyState, sphinxPacket)
		if err != nil {
...
// This test will fail with my modification and it's fine. The only difference is the seeded randomness.
func TestVectorsSendMessage(t *testing.T) {
	...
	var zeroDest [16]byte
	header, sharedSecrets, err := headerFactory.BuildHeader(route, destinationType, zeroDest)
	if err != nil {
		t.Fatalf("Unable to create forwarding header: %#v", err)
	}
	sphinxPacketFactory := NewSphinxPacketFactory(&params, pki, randReader)
	sphinxPacket, err := sphinxPacketFactory.BuildForwardSphinxPacket(route, route[len(route)-1], message, header, sharedSecrets)
	if err != nil {
		t.Fatalf("Unable to create forwarding message: %#v", err)
	}
...
func BenchmarkUnwrapSphinxPacket(b *testing.B) {
	...
	header, sharedSecrets, err := headerFactory.BuildHeader(route, route[len(route)-1][:], zeroDest)
	if err != nil {
		b.Fatalf("Unable to create forwarding header: %#v", err)
	}
	sphinxPacketFactory := NewSphinxPacketFactory(&params, pki, randReader)
	fwdMsg, err := sphinxPacketFactory.BuildForwardSphinxPacket(route, route[len(route)-1], message, header, sharedSecrets)
	if err != nil {
		b.Fatalf("Unable to create forwarding message: %#v", err)
	}

	for i := 0; i < b.N; i++ {
		_, _, err := SphinxPacketUnwrap(&params, replayCache, &keyState, fwdMsg)
		b.StopTimer()
		if err != nil {
...
func BenchmarkComposeSphinxPacket(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		sphinxPacketFactory := NewSphinxPacketFactory(&params, pki, randReader)
		_, err = sphinxPacketFactory.BuildForwardSphinxPacket(route, destID, message, header, sharedSecrets)
		if err != nil {
			b.Fatalf("unexpected an error: %v", err)
		}
	}
