...
func SphinxPacketUnwrap(params *SphinxParams, replayCache ReplayCache, privateKey PrivateKey, packet *SphinxPacket) (*UnwrappedMessage, []byte, error) {
	...
	if replayCache.Get(tag) {
		return nil, nil, ErrReplayedPacket
	}
	...
	// Like my own MACs, the function below seems to sometimes (big cap?) erase part of my variables...
	payloadTemp := make([]byte, len(payload))
	copy(payloadTemp, payload)
	lioness.XorBytes(B, append(routeInfo, padding...), cipherStream)
	copy(payload, payloadTemp)

	deltaKey, err := blockCipher.CreateBlockCipherKey(sharedSecret)
	if err != nil {
		return nil, nil, fmt.Errorf("createBlockCipherKey failure: %s", err)
	}
	delta, err := blockCipher.Decrypt(deltaKey, payload)
	if err != nil {
		return nil, nil, fmt.Errorf("123 wide block cipher decryption failure: %s", err)
	}
	...
	if messageType == MoreHops { // next hop
		...
		return nil, nil, errors.New("invalid message special destination")
	} else if messageType == ClientHop { // client
		if len(rest) < securityParameter {
			return nil, nil, fmt.Errorf("malformed client hop message")
		}
	...
	return nil, nil, fmt.Errorf("Invalid message type %d", messageType)
}
