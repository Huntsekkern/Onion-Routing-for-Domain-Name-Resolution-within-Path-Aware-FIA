...
func (f *MixHeaderFactory) BuildHeader(route [][16]byte, destination []byte, messageID [16]byte) (*MixHeader, [][32]byte, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to generate curve25519 secret: %s", err)
	}
...
// BuildForwardSphinxPacket builds a forward onion packet
func (f *SphinxPacketFactory) BuildForwardSphinxPacket(route [][16]byte, destination [16]byte, payload []byte, mixHeader *MixHeader, hopSharedSecrets [][32]byte) (*SphinxPacket, error) {
